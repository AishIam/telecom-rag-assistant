...
def check_in_scope(item):
    ...
    missing = [kw for kw in item["must_contain"] if kw.lower() not in answer_lower]
...
def check_out_of_scope(item):
    ...
    refused = (not result["in_scope"]) or (REFUSAL_MARKER in result["answer"].lower())
    if refused:
...
def main():
    ...
    print(f"Overall:                {(in_pass+out_pass)}/{n_in+n_out}  "
